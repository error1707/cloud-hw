resource "yandex_function" "style-transfer" {
  name               = "style-transfer"
  user_hash          = "hash_6"
  runtime            = "python39"
  ...
  content {
    zip_filename = "style_transfer.zip"
  }
}

resource "yandex_function_trigger" "queue-trigger" {
  name        = "queue-trigger"
  message_queue {
  ...
  function {
    id = yandex_function.style-transfer.id
    service_account_id = yandex_iam_service_account.sa.id
  }
}